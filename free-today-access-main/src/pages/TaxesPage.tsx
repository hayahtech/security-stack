...
export default function TaxesPage() {
  ...
  const cats = (categories || []).filter(c => c.group === 'Taxas');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: 'Nunito' }}>Taxas e Impostos</h1>
          <p className="text-sm text-muted-foreground">Taxas de marketplace, maquininha e impostos</p>
        </div>
        <Button onClick={() => setFormOpen(true)}><Plus className="h-4 w-4 mr-1" /> Nova Taxa</Button>
      </div>
